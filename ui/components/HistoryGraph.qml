...
Canvas {
    ...
    onPaint: {
        ...
        if(scaleMode==="Auto"){minValue=0;maxValue=Math.max(1,rawMax*1.15)} else if(scaleMode==="Peak"){const range=Math.max(1,rawMax-rawMin);minValue=Math.max(0,rawMin-range*0.18);maxValue=rawMax+range*0.18}
