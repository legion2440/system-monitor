...
ApplicationWindow {
    id:window; width:1240; height:760; minimumWidth:1040; minimumHeight:640; visible:true; title:"MoneSys"; color:Theme.bg
    property int section:0
    background:Rectangle{color:Theme.bg}
    ColumnLayout { anchors.fill:parent; spacing:0
        Rectangle { Layout.fillWidth:true;Layout.preferredHeight:Theme.titleBarH;color:Theme.chrome
            RowLayout { anchors.fill:parent;anchors.leftMargin:14;anchors.rightMargin:14;spacing:10
                Text{text:"MoneSys";color:Theme.textBright;font.family:Theme.uiFont;font.pixelSize:14;font.weight:Font.Medium}
                Rectangle{width:5;height:5;radius:3;color:app.paused?Theme.textFaint:Theme.accent;SequentialAnimation on opacity{running:!app.paused;loops:Animation.Infinite;NumberAnimation{from:1;to:0.22;duration:850}NumberAnimation{from:0.22;to:1;duration:850}}}
                Text{text:app.paused?qsTr("PAUSED"):qsTr("LIVE");color:app.paused?Theme.textFaint:Theme.accent;font.family:Theme.monoFont;font.pixelSize:9;font.letterSpacing:1.1}
                Item{Layout.fillWidth:true}
                Text{visible:app.lastError.length>0;text:app.lastError;color:"#d99a9a";elide:Text.ElideRight;Layout.maximumWidth:330;font.family:Theme.monoFont;font.pixelSize:9}
                Text{text:qsTr("poll");color:Theme.textFaint;font.pixelSize:9}
                ComboBox{Layout.preferredWidth:92;model:[250,500,1000,2000,5000];currentIndex:model.indexOf(app.pollIntervalMs);displayText:currentValue>=1000?(currentValue/1000)+" s":currentValue+" ms";delegate:ItemDelegate{required property var modelData;width:ListView.view.width;text:modelData>=1000?(modelData/1000)+" s":modelData+" ms"};onActivated:app.pollIntervalMs=currentValue}
            }
        }
        RowLayout { Layout.fillWidth:true;Layout.fillHeight:true;spacing:0
            Rectangle { Layout.preferredWidth:Theme.sidebarW;Layout.fillHeight:true;color:Theme.sidebar
            ...
            Rectangle { Layout.fillWidth:true;Layout.fillHeight:true;color:Theme.bg
                StackLayout { anchors.fill:parent;currentIndex:window.section
                    OverviewScreen{}
                    CpuScreen{}
