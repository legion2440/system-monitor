...
qulonglong AppController::ramTotal() const{return snapshot_.memory.ramTotalBytes;} qulonglong AppController::ramUsed() const{return snapshot_.memory.ramUsedBytes;} qulonglong AppController::swapTotal() const{return snapshot_.memory.swapTotalBytes;} qulonglong AppController::swapUsed() const{return snapshot_.memory.swapUsedBytes;} qulonglong AppController::diskTotal() const{return snapshot_.memory.diskTotalBytes;} qulonglong AppController::diskUsed() const{return snapshot_.memory.diskUsedBytes;} qulonglong AppController::diskAvailable() const{return snapshot_.memory.diskAvailableBytes;}
double AppController::diskUsagePercent() const{ const auto span=snapshot_.memory.diskUsedBytes+snapshot_.memory.diskAvailableBytes; if(span==0)return 0.0; return 100.0*static_cast<double>(snapshot_.memory.diskUsedBytes)/static_cast<double>(span); }
...
void AppController::setPaused(bool paused){ if(paused_==paused)return; paused_=paused; emit pausedChanged(); }
void AppController::setCollecting(bool collecting){ if(collecting_==collecting)return; collecting_=collecting; sampler_->setPaused(!collecting_); emit collectingChanged(); }
